...
code-reviewer runs `claude --print "/pr-review <target>"` directly on the host machine inside a git worktree of untrusted code. A malicious PR could craft a Makefile, pre-commit hook, or build script that executes destructive commands (e.g., `rm -rf ~/`) when Claude runs `make precommit` or other build tools during the review.

Additionally, code-reviewer currently uses `~/.claude/` config which contains the user's personal commands and agents. It should use `~/.claude-yolo/` — the same sandboxed Claude config that dark-factory uses.

## Goal

After completion, code-reviewer executes Claude reviews using `~/.claude-yolo/` config with a dedicated `/pr-review` command. The review output includes a structured JSON verdict block that the Go verdict parser extracts reliably, replacing heuristic markdown section scanning.
...
4. code-reviewer invokes `claude --print --model <model> --claude-config-dir ~/.claude-yolo "/pr-review <target>"` (or equivalent flag to use yolo config)
...
- [ ] code-reviewer uses `~/.claude-yolo` config dir when invoking Claude
...
- **Config dir flag**: No `--claude-config-dir` exists. Dark-factory uses Docker volume mount: `-v ~/.claude-yolo:/home/node/.claude`. code-reviewer should do the same — run Claude inside the `claude-yolo` container.
